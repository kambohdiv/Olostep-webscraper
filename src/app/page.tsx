...
const Page = () => {
  ...
  const downloadJson = () => {
    const jsonString = `data:text/json;charset=utf-8,${encodeURIComponent(JSON.stringify(data.dataJson, null, 2))}`;
  ...
  const downloadHtml = () => {
    const htmlString = `data:text/html;charset=utf-8,${encodeURIComponent(data.dataHtml)}`;
    const link = document.createElement('a');
    ...
    <div className="min-h-screen text-black flex flex-col items-center justify-center bg-gray-100 p-4">
      ...
      <button
        ...
        className={`mt-4 bg-blue-500 text-white py-2 px-4 rounded hover:bg-blue-600 ${
          (loading || !url || !userId || !selector) && 'opacity-50 cursor-not-allowed'
        }`}
      >
        {loading ? 'Scraping...' : 'Scrape'}
      </button>

      {loading && <p className="mt-4 text-gray-500">Scraping in progress...</p>}
